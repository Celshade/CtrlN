...
**File**: `CelKeysIO/api/auth/wallet/verify.js` (NEW)

```javascript
// POST /api/auth/wallet/verify
// 
// Verifies a Solana wallet signature and creates/retrieves user profile.
// Request body: {pubkey, nonce, signature, auth_token}
// 
// Security:
// - Verify Ed25519 signature matches pubkey + message
// - Verify nonce timestamp (max 5 minutes old)
// - Create user record if first login
// - Store profile in Redis for polling (10 min TTL, one-time retrieval)
...
const RESULT_TTL = 600  // 10 minutes (same as Matrica flow)
...
async function getUserProfile(walletAddress) {
	...
	// For now: generate profile from wallet address
	const db = getDatabase()  // Placeholder - implement with your DB
	
	let user = await db.users.findOne({ wallet_address: walletAddress })
	if (!user) {
		// First-time login: create user record
		user = {
			id: crypto.randomUUID(),
			wallet_address: walletAddress,
			username: walletAddress.slice(0, 8) + '...',  // Shortened pubkey
			avatar: null,  // Could fetch from Metaplex
...
export default async function handler(req, res) {
	...
	const { pubkey, nonce, signature, auth_token } = req.body
	...
	try {
		// Parse nonce and timestamp
		const [nonceBase64, timestamp] = nonce.split(':')
		if (!timestamp) {
		...
		const isValid = verify(pubkeyObj, messageBytes, signatureBytes)
		...
		// Generate gameState UUID (used for polling)
		const gameState = crypto.randomUUID()

		// Store profile in Redis (one-time retrieval within TTL)
		const redis = getRedis()
		await redis.set(
			`auth:result:${gameState}`,
			JSON.stringify(profile),
			'EX',
			RESULT_TTL
		)

		// Return gameState for polling
...
### Backend (CelKeysIO)
- [ ] Implement `/api/auth/wallet/verify` endpoint
- [ ] Add signature verification using @solana/web3.js
- [ ] Set up database user record creation
- [ ] Test nonce TTL enforcement (5 minute max age)
- [ ] Test with Solana devnet first
- [ ] Deploy to production (testnet or mainnet)
